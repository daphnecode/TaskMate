 ...
 */
export const submitPetExpAN3 = onCall({ region: "asia-northeast3" }, async (req) => {
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      out.steps.push(`logV2.exists=${logSnap.exists} expRewarded=${already}`);
      if (already && !force) {
        out.skipped = true;
      ...
      out.steps.push(`pet updated: before=${JSON.stringify(before)} after={"lvl":${lvl},"exp":${exp},"ups":${ups}}`);
